...
CONTRACT cardgame : public eosio::contract {
  ...
  public:
    ...
    ACTION startgame(eosio::name userName);

    ACTION endgame(eosio::name userName, bool is_force);

    ACTION playcard(eosio::name userName, uint8_t player_card_idx);

    ACTION nextround(eosio::name userName);
  ...
  private:
    ...
    void update_game_status(user& _user);

    int random(const int range);

    typedef eosio::multi_index<"users"_n, user> users_index;

    typedef eosio::multi_index<"seeds"_n, seed> seeds_index;
