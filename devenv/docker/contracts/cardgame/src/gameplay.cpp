...
int cardgame::random(const int range) {
  // Find the existing seeds

  seeds_index _seeds(_self, get_code().value);

  auto seeds_iterator = _seeds.begin();

  // Initialize the seeds with default value if it is not found
  if (seeds_iterator == _seeds.end()) {
    seeds_iterator = _seeds.emplace( _self, [&]( auto& seeds ) { });
  }

  // Generate new seeds value using the existing seeds value
  int prime = 65537;
  auto new_seeds_value = (seeds_iterator->value + now()) % prime;

  // Store the updated seeds value in the table
  _seeds.modify( seeds_iterator, _self, [&]( auto& s ) {
    s.value = new_seeds_value;
  });

  // Get the random result in desired range
  int random_result = new_seeds_value % range;
...
void cardgame::update_game_status(user& _user) {
  game& game_data = _user.game_data;
